...
import numpy as np
from nets.dbitnet import make_model as make_dbitnet
from nets.combination_1_dim import make_model as make_dbitnet_hinten
from nets.combination_2_dim import make_model as make_dbitnet_neben
from keras.callbacks import ModelCheckpoint, LearningRateScheduler
from keras.optimizers import Adam

from cipher import speck, simon


# ------------------------------------------------
# Configuration and constants
# ------------------------------------------------
logging.basicConfig(level=logging.FATAL, filename='log_training.log')
...
def train_neural_distinguisher(starting_round, data_generator, model_name, input_size, word_size, log_prefix = './', _epochs = EPOCHS, _num_samples=None, evaluate = 0, num_pairs = 1, max_round = 8, cipher = 'speck'):
    ...
    elif model_name == '1-dim-AKS':
        model = make_dbitnet_hinten(2*input_size, n_pairs = num_pairs, first_kernel_size=num_pairs)
        optimizer = tf.keras.optimizers.Adam(amsgrad=True)
    elif model_name == '1-dim-FKS':
        model = make_dbitnet_hinten(2*input_size, n_pairs = num_pairs, first_kernel_size= 2)
        optimizer = tf.keras.optimizers.Adam(amsgrad=True)
    elif model_name == '2-dim-NPP':
        model = make_dbitnet_neben(2*input_size, n_pairs = num_pairs, preprocess = False)
        optimizer = tf.keras.optimizers.Adam(amsgrad=True)
    elif model_name == '2-dim-PP':
        model = make_dbitnet_neben(2*input_size, n_pairs = num_pairs, preprocess= True)
        optimizer = tf.keras.optimizers.Adam(amsgrad=True)
    ...
    while True:
        ...
        X_val, Y_val = data_generator(num_samples//10, current_round)
        ...
        message += f'{model_name}, round {current_round}. Best validation accuracy: {val_acc}\n'
        ...
        # otherwise save results as currently best reached round
            ...
            if evaluate != 0:
                if cipher == 'speck':
                    X_combine, Y_combine = speck.make_train_data(plain_bits, key_bits, num_samples//10, current_round, delta_plain, delta_key, evaluate)
                elif cipher == 'simon':
                    X_combine, Y_combine = simon.make_train_data(plain_bits, key_bits, num_samples//10, current_round, delta_plain, delta_key, evaluate)
                combined_val = evaluate_net(model, X_combine, Y_combine, evaluate)
                message += f'{model_name}, round {current_round}. Best combined validation accuracy: {combined_val}\n'
            best_round = current_round
            best_val_acc = val_acc
            logging.info(f"ABORT TRAINING max round reached (best validation accuracy {val_acc}<={ABORT_TRAINING_BELOW_ACC}).")
            break
        else:
            if evaluate != 0:
                if cipher == 'speck':
    ...
    return best_round, best_val_acc, message




#speck.make_train_data(encryption_function, plain_bits, key_bits, num_samples, nr, delta_plain, delta_key),


def evaluate_net(net, evaluation, ground_truth, nr_pairs):
    eval_length = len(ground_truth)
    prob = np.ones(eval_length)
    for i in range(nr_pairs):
        test = net.predict(evaluation[: , i *64: (i+1) *64])
        calc = 1 - test
        test += 0.000000000001
        for j in range(eval_length):
            prob[j] *= calc[j] / test[j]
    prob += 1
    prob = np.round(1 / prob)

    eval = (prob == ground_truth).sum()
    return eval/ eval_length
