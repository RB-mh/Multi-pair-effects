...
import numpy as np
import pandas as pd
from cipher import simon, speck
...
def optimize(plain_bits, key_bits, encryption_function, nb_samples=NUM_SAMPLES, scenario = "single-key", log_file = None, epsilon=0.1, i = 1):
    allDiffs = None
    totalScores = {}
    diffs = None
    ...
    for nr in range(1, current_round):
        ...
        entries = 20
        result, _, _ = PrettyPrintBestNDifferences(allDiffs, finalScores[nr], entries, scenario, plain_bits, key_bits)
        resStr = f'Best at {nr}: \n{result}'
        if log_file != None:
            with open(log_file, 'a') as f:
                f.write(resStr)
    ...
    df.to_csv(f'{log_file}{i}_best_weighted_differences.csv')
...
if __name__ == '__main__':
    ...
    if cipher == 'speck':
        ...
        encryption_function = speck.encrypt
    else:
        plain_bits = simon.plain_bits
        key_bits = simon.key_bits
        encryption_function = simon.encrypt

    for j in range(5):
        optimize(plain_bits, key_bits, encryption_function, nb_samples=NUM_SAMPLES, scenario = "single-key", log_file = cipher, epsilon=0.1, i=j)
